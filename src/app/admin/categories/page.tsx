    ...
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <AdminPanel title="Add a category" bodyClassName="p-5">
          ...
          >
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Category name" className="mc-input" />
            <button type="submit" disabled={!name.trim()} className="mc-btn shrink-0">Create</button>
          </form>
            ...
            <ul className="divide-y divide-line">
              {categories.map((c) => (
                <li key={c.id} className="flex items-center justify-between gap-3 px-5 py-3.5">
                  ...
                  <button type="button" onClick={() => deleteCategory(c.id)} className="mc-btn-ghost h-9 px-3 text-sm text-muted hover:text-accent">
                    Delete
                  </button>
                </li>
              ))}
