  ...
  if (!userId) {
    return (
      <div className="mc-container py-20 text-center">
        ...
        <p className="mt-3 text-muted-foreground">Sign in to see your order history.</p>
        <Button asChild className="mt-6"><Link href="/login?next=/orders">Sign in</Link></Button>
      </div>
    );
  }
    ...
    <div className="mc-container py-12 md:py-16">
      ...
      {orders.length === 0 ? (
        <div className="mt-10 rounded-[var(--r-lg)] border border-dashed border-line-strong bg-surface px-6 py-16 text-center">
          ...
          <Button asChild className="mt-6"><Link href="/products">Find a gift</Link></Button>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-line">
                {orders.map((o: any) => (
                  <tr key={o._id.toString()} className="transition-colors hover:bg-surface/60">
                    ...
                    <td className="px-5 py-4 capitalize text-muted-foreground">{o.paymentStatus}</td>
                    <td className="px-5 py-4 font-semibold text-ink">{formatLkr(Number(o.total))}</td>
                    <td className="px-5 py-4 text-muted-foreground">{new Date(o.createdAt).toLocaleDateString('en-LK', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
